...
# ---------------------------------------------------------------------------
# LEAGUE TABLE – FORM column removed, high-res
# ---------------------------------------------------------------------------
...
def _render_table_image(table_data, is_complete):
    cols = ["POS", "TEAM", "MP", "W", "D", "L", "GF", "GA", "GD", "PTS"]  # FORM removed
...
# ---------------------------------------------------------------------------
# FIXTURES – no VS, winner highlighting, high-res
# ---------------------------------------------------------------------------
...
def _render_fixtures_image(fixtures, player, context, status, page, total_pages):
    ...
    for idx, fixture in enumerate(fixtures):
        ...
        if home_score and away_score and home_score.isdigit() and away_score.isdigit():
            ...
            score_color = config.THEME_ACCENT_GOLD if h > a else config.THEME_ACCENT_RED if a > h else config.THEME_ACCENT_AMBER
    ...
    draw.text((padding, total_height - 18), venue_text, fill=config.THEME_TEXT_MUTED, font=row_font)
    img_bytes = io.BytesIO()
    img.save(img_bytes, format='PNG')
    img_bytes.seek(0)
    return img_bytes
...
# ---------------------------------------------------------------------------
# MATCHDAY IMAGE – same improvements
# ---------------------------------------------------------------------------
